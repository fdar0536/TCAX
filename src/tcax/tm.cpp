    ...
    for (i = 0; i < pSyllable->syls; i ++) {
        if (L'\0' == pSyllable->syl2D[i]) {
            ...
            if (0 == index) {
                ...
                szUni[1] = L'\0';
                std::wstring src_str(szUni);
                std::string dst_str = boost::locale::conv::utf_to_utf<char>(src_str);
...
void libtm_fill_tm_word_hori(const SYL_pWord pWord, const TM_pTextHori pTextHori, TM_pWordHori pWordHori) {
...
void libtm_fill_tm_word_vert(const SYL_pWord pWord, const TM_pTextVert pTextVert, TM_pWordVert pWordVert) {
...
void libtm_fill_tm_adv_hori(const SYL_pAdv pAdv, const TM_pTextHori pTextHori, TM_pAdvHori pAdvHori) {
    int i, j, k, len, count, height;
...
void libtm_fill_tm_adv_vert(const SYL_pAdv pAdv, const TM_pTextVert pTextVert, TM_pAdvVert pAdvVert) {
    int i, j, k, len, count, width;
    int *advSylCount2D;    /* number of texts (syllables) in a adv word */
