...
TCC_Error_Code libtcc_read_file_to_unicode(const char *filename, wchar_t **pBuffer, int *pCount) {
    ...
    rawBuffer[size + 1] = '\n';
    rawBuffer[size + 2] = '\0';
    if (fread(rawBuffer, sizeof(unsigned char), size, fp) != (int)size) {
        free(rawBuffer);
        fclose(fp);
        return tcc_error_file_while_reading;
    }
...
TCC_Error_Code libtcc_write_tcc_string_to_file(const char *filename, const wchar_t *tccString) {
    FILE *fp;
    int size;
    char *utf8String;
    unsigned char BOM[3];
...
TCC_Error_Code libtcc_create_default_tcc_file(const char *filename) {
    ...
    error = libtcc_write_tcc_string_to_file(filename, tccString);
