...
const char *tcax_get_tcc_directory(const char *tccFilename, char **pTccDirectory)
{
    ...
    char *tccDirectory;
    len = strlen(tccFilename);
    count = std::max(len, 4095) + 2; //one more for '/'
    fullTccFilename = (char *)malloc(count * sizeof(char));
    if ('/' != tccFilename[0])
    {
        int size;
        ...
        tmp_char[0] = '/';
        memcpy(fullTccFilename + size, tmp_char, sizeof(char));
        memcpy(fullTccFilename + size + 1, tccFilename, (len + 1) * sizeof(char));
        delete[] tmp_char;
    }
    else
        memcpy(fullTccFilename, tccFilename, (len + 1) * sizeof(char));
    ...
    if (i < 0) //cannot find any matched pattern
    {
        count = len;
    }
    else
...
#ifndef _WIN32
/* This function took from stackoverflow. */
...
static void _tcax_make_full_path_of_font_file(const wchar_t *tccDirectory, TCC_pAttributes pAttributes)
{
    if (wcsstr(pAttributes->font_file, L":") == NULL)
    {
        ...
        str1 = std::wstring(pAttributes->font_file);
        str2 = std::wstring(tccDirectory);

        str3 = boost::locale::conv::utf_to_utf<char>(str1); //font_file
        str4 = boost::locale::conv::utf_to_utf<char>(str2);

        if (file_isreg((str3 + "/" + str4).c_str()) != 1) //invalid file
        {
            //try home path
            struct passwd *pw = getpwuid(getuid());
            const char *homedir = pw->pw_dir;
            str4 = std::string(homedir) + "/.local/share/fonts/" + str3;
            if (file_isreg(str4.c_str()) != 1)
            {
                str4 = "/usr/share/fonts/" + str3;
            }
            str1 = boost::locale::conv::utf_to_utf<wchar_t>(str4);

            wchar_t *tempString = pAttributes->font_file;
            delete[] tempString;

            pAttributes->font_file = new wchar_t[str1.size() + 1]();
            memcpy(pAttributes->font_file, str1.c_str(), str1.size() * sizeof(wchar_t));
            pAttributes->font_file[str1.size()] = L'\0';
        }
        else
        {
            str1 = str2 + L"/" + str1;
...
int tcax_init_tcaxpy_data(const char *tccFilename, const char *outFilename, TCC_pAttributes pAttributes, PY_pInitData pInitData) {
    ...
    if (K_MODE_WORD == pAttributes->k_mode) {
        ...
        libtm_fill_tm_word_hori(&sylWord, &tmTextHori, &tmWordHori);
        ...
        libtm_fill_tm_word_vert(&sylWord, &tmTextVert, &tmWordVert);
        ...
        libtm_fill_tm_adv_hori(&sylAdv, &tmTextHori, &tmAdvHori);
        ...
        libtm_fill_tm_adv_vert(&sylAdv, &tmTextVert, &tmAdvVert);
...
#else
int tcax_write_ass_string_to_file(FILE *assfp, const wchar_t *assString) {
#endif
    int size;
    char *utf8String;
#if defined WIN32
...
#else
    tcax_write_ass_string_to_file(assfp, pInitData->assHeader);
#endif
    for (i = 0; i < pAttributes->py_file_count; i ++) {
        if (pAttributes->beg_line[i] <= 0)
            pAttributes->beg_line[i] += pInitData->sylData.lines;
        ...
        for (iLine = 0; iLine < pInitData->sylData.lines; iLine ++) {
            ...
            for (iText = 0; iText < nTexts; iText ++) {
                ...
                if (iLine >= pAttributes->beg_line[i] && iLine < pAttributes->end_line[i] && !libsyl_is_white_spaces(text, wcslen(text))) {
                    ...
                    if ((intptr_t)pReturnedBuf[0] > 0) {
#ifdef _WIN32
                        tcax_write_ass_string_to_file(assfp, (const wchar_t *)pReturnedBuf[1], (intptr_t)pReturnedBuf[0]);
#else
                        tcax_write_ass_string_to_file(assfp, (const wchar_t *)pReturnedBuf[1]);
#endif
                        free(pReturnedBuf[1]);
                    }
                    if ((intptr_t)pReturnedBuf[2] > 0) {
                        free(pReturnedBuf[3]);
                    }
                    free(pReturnedBuf);
                    printf("Executing script %i of %i, progress: %.2f%%\r", i + 1, pAttributes->py_file_count, 100 * (index - offset) / (double)total);
                    fflush(stdout);
                }
                index ++;
            }
